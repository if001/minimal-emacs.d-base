  ...
  (setq org-capture-templates
	'(
      ...
      ("r" "Rad" entry (file my/get-memo-file-path)
       "** %? :rad: \n:PROPERTIES:\n:CREATED: %U\n:TAG: rad\n:END:\n%i\n" :empty-lines 1 :tree-type day)
      ("w" "Rad with Track" entry (file my/get-memo-file-path)
       "** タスク :rad:track: \n:PROPERTIES:\n:CREATED: %U\n:TAG: rad\n:END:\n%i\n" :empty-lines 1 :tree-type day)
      ("s" "Summary(集計)" entry (file my/get-memo-file-path)
       "** 集計 :summary: \n#+BEGIN: clocktable :scope file :match \"track\" :properties (\"CREATED\")\n #+END:\n%i%?\n" :empty-lines 1 :tree-type day)
